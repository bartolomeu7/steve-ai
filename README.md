...
literal e o futuro Security Center chegam nas próximas versões.
...
O botão "📊 Dashboard" (ao lado de ⚙ Configurações) abre um segundo painel com quatro
...
nome) e **Steve Core** (um feed estilizado que reage a eventos reais do Steve — pedido à
IA, resposta, ferramenta em execução, voz). Fechar o Dashboard não encerra o Steve, e
reabri-lo nunca cria um segundo coletor. É só monitoramento: nenhuma ação de matar/
...
tools/        ferramentas seguras (nível LOW): apps, processos, CPU/RAM/disco, arquivos, URLs
security/     permissões, confirmações e auditoria
...
ui/desktop/dashboard/  System Dashboard (Overview/Performance/Processes/Steve Core)
tests/        suíte pytest
```
